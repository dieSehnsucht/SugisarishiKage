...
screen say(who, what):
    ...
    if tag is not None and renpy.has_image(tag):
        add tag at right  # 去掉 zorder 0
...
screen navigation():
    ...
    if main_menu:
        hbox:
            xalign 0.5
            yalign 1.0   # 改为1.0，按钮在最下方
            spacing 80

            textbutton _("开始") action Start()
            textbutton _("继续") action Continue()
            textbutton _("读取") action ShowMenu("load")
            textbutton _("设置") action ShowMenu("preferences")
    ...
    use game_menu(_("历史"), scroll=("vpgrid" if gui.history_height else "viewport"), yinitial=1.0, spacing=gui.history_spacing):
        ...
        for h in _history_list:
            ...
            window:
                ...
                has fixed:
                    yfit gui.history_height is None
                ...
                # 新增跳转按钮
                if h.rollback_identifier is not None:
                    textbutton "跳转":

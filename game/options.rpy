...
define config.name = _("過ぎ去りし影")

define config.default_text_cps = 0


# 设置未读对白为黑色，已读为灰色
define gui.text_color = "#000000"         # 未读对白颜色（黑色）
define gui.text_read_color = "#888888"    # 已读对白颜色（灰色，可自定义）

# 如果你用的是自定义 say 窗口，也可以这样写
...
define build.name = "SugisarishiKage"
define build.directory_name = "SugisarishiKage"
define build.executable_name = "SugisarishiKage"
## 音效和音乐 #######################################################################
...
define config.save_directory = "SugisarishiKage-1757309460"
...
init python:
    ...
    build.classify('game/audio/**', "resource")
    build.classify('game/video/**', "resource")
    build.classify('game/**.png', "resource")
    build.classify('game/**.jpg', "resource")
    build.classify('game/**.ico', "resource")
    build.classify('game/gui', "resource")
    build.classify('game/chapters', "resource")
